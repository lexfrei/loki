...
type Worker struct {
	...
	schedulerListener wire.Listener // only used for local transport
...
func New(config Config) (*Worker, error) {
	...
	var (
		workerListener    wire.Listener
		schedulerListener wire.Listener
	)
...
func (w *Worker) run(ctx context.Context) error {
	...
	} else if w.schedulerListener != nil {
...
func (w *Worker) dial(ctx context.Context, addr net.Addr) (wire.Conn, error) {
	...
	case wire.LocalScheduler:
		...
		return w.schedulerListener.(*wire.Local).DialFrom(ctx, w.workerListener.Addr())
